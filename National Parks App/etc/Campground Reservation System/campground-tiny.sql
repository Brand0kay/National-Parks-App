...
-- Delete the NPCampsite Database (IF EXISTS)
IF EXISTS(select * from sys.databases where name='NPCampsite')
DROP DATABASE NPCampsite;
GO

-- Create a new NPCampsite Database
CREATE DATABASE NPCampsite;
GO

-- Switch to the NPCampsite Database
USE NPCampsite
GO
...
select * from reservation
 ...
 where reservation.from_date between '2018-10-29' and '2018-11-30' and reservation.to_date between '2018-10-29' and '2018-11-30' and campground.campground_id = 2 and reservation.name = null

 select * from site 
 inner join reservation on site.site_id = reservation.site_id
 inner join campground on site.campground_id = campground.campground_id
 where reservation.from_date between '2018-10-29' and '2018-11-30' and reservation.to_date between '2018-10-29' and '2018-11-30' and campground.campground_id = 2
 
 select * from site

 SELECT * FROM site
 inner join campground on site.campground_id = campground.campground_id
  WHERE campground.campground_id = @campgroundCode
